...
class RateModel {
  ...
  // Calculate price (dengan service_type)
  static calculatePrice(courierId, origin, destination, service_type, weight, callback) {
    const sql = `SELECT price_per_kg 
                 FROM shipping_rates 
                 WHERE courier_id = ? AND origin = ? AND destination = ? AND service_type = ?
                 LIMIT 1`;
    db.query(sql, [courierId, origin, destination, service_type], (err, results) => {
